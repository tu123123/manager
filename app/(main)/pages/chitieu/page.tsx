...
import { messchitieuStore } from '@/store/messchitieu';
const dbPhieunhap = 'donchitieu';
const DetailConngno = ({ onClose, listCongno }: { listCongno: phieunhapItemtype[]; onClose: () => void }) => {
    const listtoday=listCongno?.filter(x=>moment(x.time).format('DD/MM/YYYY')==moment(listCongno[listCongno.length-1]?.time).format('DD/MM/YYYY'))
     const listnotoday=listCongno?.filter(x=>moment(x.time).format('DD/MM/YYYY')!=moment(listCongno[listCongno.length-1]?.time).format('DD/MM/YYYY'))
...
export default function Congno() {
    const { getmessPhieunhap,phieunhapmess } = messchitieuStore();
