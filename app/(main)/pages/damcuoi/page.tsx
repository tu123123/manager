...
import { dbkhachmoi, dskhachmoistype, chiphidamcuoiStore } from '@/store/chiphidamcuoi';
...
export default function Khachmoi() {
    const { dsKhachmoi, getDsKhachmoi } = chiphidamcuoiStore();
        ...
        <div className="khachhang-container">
            ...
            >
              ...
              <div style={{
                ...
                }}>  <TriStateCheckbox invalid value={status} onChange={(e) => setStatus(e.value)} />{status===null?'Tất cả':status?'Nhà gái':'Nhà trai'}</label>
                <Input style={{
                    ...
                    return (
                        ...
                        >
                            <div style={{
                                ...
                                alignItems:'center',
                                gap:'10px'
                            }}><Tag severity={i.status?'success':'info'}>{i.status?"Nhà gái":'Nhà trai'}</Tag>{i.ten} </div>
            ...
            <div className='total'>
                Tổng: {formatNumber(dsKhachmoi.filter(x=>x.status==status||status===null).reduce((a,b)=>a+b.sotien,0))}
            </div>
        </div>
    );
}
